...
function App() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [cardResponse, favoritesResponse, sneakersResponse] =
          await Promise.all([
            axios.get("https://610002adbca46600171cf698.mockapi.io/cards"),
            axios.get("https://610002adbca46600171cf698.mockapi.io/favorites"),
            axios.get("https://610002adbca46600171cf698.mockapi.io/items"),
          ]);
  ...
  const onAddToCart = async (obj) => {
    const item = cartSneakers.find(
      (item) => Number(item.itemid) === Number(obj.id)
    );
    try {
      if (item) {
        axios.delete(
          `https://610002adbca46600171cf698.mockapi.io/cards/${item.id}`
        );
        ...
        const { data } = await axios.post(
          "https://610002adbca46600171cf698.mockapi.io/cards",
          obj
        );
  ...
  const onAddFavorite = async (obj) => {
    try {
      ...
      if (item) {
        axios.delete(
          `https://610002adbca46600171cf698.mockapi.io/favorites/${item.id}`
        );
        setFavorites((prev) =>
          prev.filter((item) => Number(item.itemid) !== Number(obj.id))
        );
        ...
        const { data } = await axios.post(
          "https://610002adbca46600171cf698.mockapi.io/favorites",
          obj
        );
